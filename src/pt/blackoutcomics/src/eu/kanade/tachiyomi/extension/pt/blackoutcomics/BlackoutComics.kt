        ...
        network.client.newBuilder()
            .addInterceptor { chain ->
                ...
                if (response.request.url.pathSegments.contains("temp")) {
                    return@addInterceptor doAuth(response, chain)
                }
                response
            }
    ...
    private fun doAuth(
        ...
        chain.proceed(POST("$baseUrl/blackout/login", headers, form)).use {
            if (it.request.url.pathSegments.contains("temp")) {
                throw IOException(
                    """
                    Configure suas credencias em Extensões > $name > Sttings.
                    Adicione usuário e senha.
                    """.trimIndent(),
                )
            }
        }
        return chain.proceed(popularMangaRequest(0))
    }
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        val usernamePref = EditTextPreference(screen.context).apply {
            ...
            setOnPreferenceChangeListener { _, newValue ->
                preferences.edit()
                    .putString(USERNAME_PREF, newValue as String)
                    .apply()
                true
            }
        }
        ...
        val passwordPref = EditTextPreference(screen.context).apply {
            ...
            setOnPreferenceChangeListener { _, newValue ->
                preferences.edit()
                    .putString(PASSWORD_PREF, newValue as String)
                    .apply()
                true
            }
        }
